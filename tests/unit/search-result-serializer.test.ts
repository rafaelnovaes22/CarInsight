...
} from '../../src/services/search-result-serializer.service';
import {
    ExactSearchResult,
 ...
 */
const resultTypeGenerator = fc.constantFrom<ExactSearchResultType>('exact', 'year_alternatives', 'suggestions', 'unavailable');

/**
 * Generator for match types
 */
const matchTypeGenerator = fc.constantFrom<'exact' | 'year_alternative' | 'suggestion'>('exact', 'year_alternative', 'suggestion');
 ...
 */
const availableYearsGenerator = fc.array(yearGenerator, { minLength: 0, maxLength: 10 })
...
describe('SearchResultSerializer Property Tests', () => {
    ...
    describe('Property 9: Serialization round-trip preserves data integrity', () => {
            ...
            fc.assert(
                fc.property(
                    ...
                    (result) => {
                        ...
                        expect(['exact', 'year_alternatives', 'suggestions', 'unavailable']).toContain(parsed.type);
                    }
                ),
                { numRuns: 100 }
            );
        ...
        it('deserialize throws on missing required fields', () => {
            // Test missing type
            expect(() => serializer.deserialize(JSON.stringify({
                vehicles: [],
                message: 'test',
        ...
        it('deserialize throws on invalid type value', () => {
            expect(() => serializer.deserialize(JSON.stringify({
                ...
                vehicles: [],
                message: 'test',
                metadata: { requestedModel: 'Onix', requestedYear: 2020, timestamp: new Date().toISOString() }
            }))).toThrow();
        });
