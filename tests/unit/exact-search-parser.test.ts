...
import { ExactSearchParser, KNOWN_MODELS, ExtractedFilters } from '../../src/services/exact-search-parser.service';
 ...
 */
const modelYearQueryGenerator = fc.tuple(modelGenerator, fullYearGenerator)
    .map(([model, year]) => `${model} ${year}`);

/**
 * Generator for query format: "year model" (e.g., "2019 Onix")
 */
const yearModelQueryGenerator = fc.tuple(fullYearGenerator, modelGenerator)
    .map(([year, model]) => `${year} ${model}`);

/**
 * Generator for query format with abbreviated year: "model YY" (e.g., "Onix 19")
 */
const modelAbbrevYearQueryGenerator = fc.tuple(modelGenerator, abbreviatedYearGenerator)
 ...
 */
const yearRangeQueryGenerator = fc.tuple(
    modelGenerator,
 ...
 */
const slashYearQueryGenerator = fc.tuple(
    modelGenerator,
...
describe('ExactSearchParser Property Tests', () => {
    ...
    describe('Property 1: Parser extracts model and year correctly from all valid formats', () => {
        ...
        it('returns null for model when no known model is present', () => {
            ...
            const noModelQueryGenerator = fc.array(fc.constantFrom('a', 'b', 'c', ' ', '1', '2', '3'), { minLength: 1, maxLength: 20 })
