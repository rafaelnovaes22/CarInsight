...
// Mock logger to avoid noise if possible, but exactSearchParser imports it directly.
// We rely on console.log for our test output.
...
async function runTest() {
    ...
    try {
        ...
        // 5. Verification
        if (result.model && result.model.toLowerCase() === fakeModel.toLowerCase()) {
            console.log('✅ SUCCESS: Dynamic model was recognized!');
        } else {
            console.error(`❌ FAILURE: Model was NOT recognized.`);
            console.error(`   Expected: '${fakeModel}'`);
            console.error(`   Got:      '${result.model}'`);
            console.error(`   Raw Query: '${result.rawQuery}'`);
            process.exit(1);
        }
