    ...
    try {
      ...
      */
      ...
      if (
        ...
        // Check if we ACTUALLY have motos in stock before rejecting
        const motoCheck = await vehicleSearchAdapter.search('moto', { limit: 1 });
        const hasMotos = motoCheck.length > 0;
        ...
        if (!hasMotos) {
          ...
          logger.info('Moto requested and found in inventory - Allowing flow to proceed');
          updatedProfile.bodyType = 'moto';
        }
      }
...
Quer que eu mostre opções de SUVs ou sedans espaçosos de 5 lugares como alternativa?`;
  ...
  private async getRecommendations(profile: Partial<CustomerProfile>): Promise<{
    ...
    try {
      ...
      // Link post-filters to the already seat-filtered results
      ...
      if (filteredResults.length > 1) {
        try {
          ...
          return { recommendations: ranked as VehicleRecommendation[], wantsPickup };
        } catch (e) {
