...
Quer que eu mostre opções de SUVs ou sedans espaçosos de 5 lugares como alternativa?`;
  ...
  private async getRecommendations(
    ...
    try {
      ...
      // Strong keywords: unambiguous, always indicate pickup intent
      const strongPickupKeywords = [
        'pickup',
        'picape',
        'caminhonete',
        'caçamba',
      ...
      // Weak keywords: ambiguous, only match as whole words with word boundaries
      const weakPickupKeywords = [
        'carga',
        'obra',
        'material',
        'construção',
      ...
      const hasPickupInText =
        strongPickupKeywords.some(kw => searchTextLower.includes(kw)) ||
        weakPickupKeywords.some(kw => matchesWord(searchTextLower, kw));
      ...
      const hasWorkUsage = matchesWord(usageText, 'trabalho') || matchesWord(usageText, 'obra');

      // Check priorities array for any pickup-related terms
      const prioritiesText = (profile.priorities || []).join(' ').toLowerCase();
      const allPickupKeywords = [...strongPickupKeywords, ...weakPickupKeywords];
      const hasPickupInPriorities = allPickupKeywords.some(kw => matchesWord(prioritiesText, kw));

      const wantsPickup =
        profile.bodyType === 'pickup' ||
        hasPickupInText ||
        hasPickupInPriorities ||
        (hasWorkUsage && allPickupKeywords.some(kw => matchesWord(usageText, kw)));
