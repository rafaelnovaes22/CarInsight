...
export const SHORT_AFFIRMATIVES: readonly string[] = [
    'sim', 's', 'ss', 'sss', 'siiim', 'siim',
    'ok', 'okay', 'blz', 'bora', 'show',
    'ta', 'tá', 'claro', 'pode', 'quero',
    'manda', 'mostra', 'beleza', 'tranquilo',
    'vamos', 'certeza', 'pf', 'pfv'
];
...
const normalize = (message: string): string => {
    return message.toLowerCase().trim().replace(/[.,!?]+$/, '').trim();
};
