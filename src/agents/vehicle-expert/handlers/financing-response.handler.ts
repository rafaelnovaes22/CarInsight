...
const extractTradeInVehicle = (message: string): { model?: string; year?: number; brand?: string } | null => {
    ...
    const vehicleMatch = message.match(/\b(?:tenho|meu|minha|um|uma|e\s+um|mais\s+um)\s+(?:um\s+)?([\wà-ü-]+)\s+(\d{4})\b/i);
    if (vehicleMatch) {
...
_Digite "vendedor" para falar com nossa equipe!_`,
    ...
    const tradeInName = tradeInVehicle
        ? `${tradeInVehicle.brand || ''} ${tradeInVehicle.model || ''} ${tradeInVehicle.year || ''}`.trim() || 'seu veículo'
        : 'seu veículo';
