...
function detectCombustivel(nome, ano) {
  ...
  if (nomeLower.includes('125i') || nomeLower.includes('528') || nomeLower.includes('evoque'))
    return 'Gasolina';
  return 'Flex';
}
...
async function main() {
  ...
  for (const vehicle of VEHICLES_LIST) {
    ...
    process.stdout.write(
      `\r🔄 ${processed}/${VEHICLES_LIST.length}: ${vehicle.nome.substring(0, 35)}...     `
    );
  ...
  if (withPrice.length > 0) {
    ...
    console.log(
      `💰 Faixa de preço: R$ ${Math.min(...prices).toLocaleString('pt-BR')} - R$ ${Math.max(...prices).toLocaleString('pt-BR')}`
    );
  }
