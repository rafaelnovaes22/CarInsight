  ...
  try {
    ...
    if (choice.trim() === '2') {
      const phoneNumber = await askQuestion(
        'Digite o número do WhatsApp com DDD (ex: 5511999999999): '
      );
      if (!phoneNumber) {
...
async function checkOrCreateInstance(headers: any) {
  ...
  try {
    ...
    const instance = Array.isArray(instances)
      ? instances.find(
          (i: any) => i.instance?.instanceName === INSTANCE_NAME || i.name === INSTANCE_NAME
        )
      : null;
  ...
  try {
    ...
    if (qrCode) {
      ...
      if (qrCode.startsWith('data:image')) {
        ...
        console.log(
          'Abra http://localhost:8080/manager para escanear, ou tente a opção Pairing Code.'
        );
      } else {
...
async function connectViaPairingCode(headers: any, number: string) {
  ...
  for (const [i, query] of queries.entries()) {
    try {
      ...
      const res = await axios.get(`${API_URL}/instance/connect/${INSTANCE_NAME}?${queryString}`, {
        headers,
      });
