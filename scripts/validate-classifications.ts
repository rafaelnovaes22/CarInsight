...
async function main() {
    ...
    for (const v of vehicles) {
        ...
        const isCargoBody = ['pickup', 'picape', 'furg', 'van', 'caminhonete'].some(t => body.includes(t));
        if (v.aptoCarga && !isCargoBody) {
            report.aptoCarga.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoCarga but body is '${v.carroceria}'`);
        }

        // --- aptoEntrega Rules ---
        // Expected: >= 2010, functional
        if (v.aptoEntrega) {
            if (v.ano < 2010) report.aptoEntrega.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoEntrega but year ${v.ano} < 2010`);
        ...
        if (v.aptoUber) {
            if (v.ano < 2012) report.aptoUber.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUber but year ${v.ano} < 2012`);
            if (v.portas < 4) report.aptoUber.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUber but has ${v.portas} doors`);
            if (!v.arCondicionado) report.aptoUber.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUber but no AC`);
        }

        // --- aptoUberBlack Rules ---
        // Expected: >= 2018 (sedan) or >= 2019 (suv), 4 doors, AC. Hatchbacks excluded.
        if (v.aptoUberBlack) {
            if (v.ano < 2018) report.aptoUberBlack.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUberBlack but year ${v.ano} too old`);
            if (body.includes('hatch')) report.aptoUberBlack.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUberBlack but is HATCH`);
            if (v.portas < 4) report.aptoUberBlack.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUberBlack but has ${v.portas} doors`);
        }

        // --- aptoFamilia Rules ---
        // Expected: 4 doors, Space (Sedan, SUV, Minivan, Perua) or large hatch?
        // Current Strict Rule: Exclude small hatches
        const isFamilyBody = ['sed', 'suv', 'minivan', 'van', 'perua', 'wagon'].some(t => body.includes(t));
        // Small hatches check (Mobi, Kwid, Up) - usually not family
        const isSmallHatch = ['mobi', 'kwid', 'up', 'picanto', 'celta', 'gol', 'uno', 'ka'].some(m => model.includes(m));

        if (v.aptoFamilia) {
            if (v.portas < 4) report.aptoFamilia.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoFamilia but has ${v.portas} doors`);
            if (!isFamilyBody && isSmallHatch) report.aptoFamilia.mismatches.push(`[QUESTIONABLE] ${name}: Marked aptoFamilia but is small hatch '${v.modelo}'`);
        }

        // --- aptoUsoDiario Rules ---
        // Expected: Economical, AC
        if (v.aptoUsoDiario) {
            if (!v.arCondicionado) report.aptoUsoDiario.mismatches.push(`[FALSE POSITIVE] ${name}: Marked aptoUsoDiario but no AC`);
            // Checking for "beberroes"
            if (body.includes('camaro') || body.includes('mustang') || (body.includes('pickup') && !model.includes('strada') && !model.includes('saveiro'))) {
                report.aptoUsoDiario.mismatches.push(`[QUESTIONABLE] ${name}: Marked aptoUsoDiario but might be guzzler`);
            }
        }
    }
