...
async function main() {
  ...
  // Header
  console.log(
    `${'ID'.padEnd(3)} | ${'VEÍCULO'.padEnd(40)} | ${'CAT'.padEnd(8)} | ${'ANO'.padEnd(4)} | ${'PREÇO'.padEnd(10)} | ${'FAM'.padEnd(3)} | ${'UBER'.padEnd(4)} | ${'BLACK'.padEnd(5)}`
  );
  ...
  vehicles.forEach((v, idx) => {
    ...
    console.log(
      `${(idx + 1).toString().padEnd(3)} | ${name.padEnd(40)} | ${cat.padEnd(8)} | ${v.ano.toString().padEnd(4)} | ${price.padEnd(10)} | ${fam.padEnd(3)} | ${uber.padEnd(4)} | ${black.padEnd(5)}`
    );
  });
