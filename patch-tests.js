...
if (fs.existsSync(e2ePath)) {
    ...
    content = content.replace(/const result = service\.validateInput/g, "const result = await service.validateInput");
...
if (fs.existsSync(mpPath)) {
    ...
    content = content.replace(/validateInput: vi\.fn\(\)\.mockReturnValue\(\{ allowed: true/g, "validateInput: vi.fn().mockResolvedValue({ allowed: true");
...
if (fs.existsSync(perfPath)) {
    ...
    // replace `it('...', () => {` with `it('...', async () => {`
    content = content.replace(/it\('should validate input in less than 5ms', \(\) => {/g, "it('should validate input in less than 5ms', async () => {");
    content = content.replace(/it\('should detect prompt injection in less than 10ms', \(\) => {/g, "it('should detect prompt injection in less than 10ms', async () => {");
    content = content.replace(/it\('should handle 100 validations in less than 500ms', \(\) => {/g, "it('should handle 100 validations in less than 500ms', async () => {");
    content = content.replace(/it\('should have p95 latency under 5ms for guardrail validation', \(\) => {/g, "it('should have p95 latency under 5ms for guardrail validation', async () => {");

    content = content.replace(/const result = guardrails\.validateInput/g, "const result = await guardrails.validateInput");
